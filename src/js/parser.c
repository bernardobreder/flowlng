...
struct js_node_t* js_parser_element_function(struct js_parser_t* self) {
    ...
    js_node_def_token(id_token);
    if (js_parser_type_not(JS_TOKEN_ID)) {
        return js_parser_error("function: function '<id>' expected", id_token, 0);
    }
    js_parser_id_def(id);
    js_parser_next();
    ...
    if (js_parser_type_not('(')) {
        ...
        return js_parser_error("function: function <id> '(' expected", arg_open_token, 0);
    }
    js_parser_next();
    ...
    if (parameters && js_node_error_is(parameters)) {
        js_parser_node_id_free(id);
        return js_parser_error("function: function <id> ( '<parameter list>' expected", arg_token, parameters);
    }
    ...
    if (js_parser_type_not(')')) {
        ...
        return js_parser_error("function: function <id> ( <parameter list> ')' expected", arg_close_token, 0);
    }
    js_parser_next();
    ...
    if (js_node_error_is(statement)) {
        js_parser_node_id_free(id);
        js_node_free(parameters);
        return js_parser_error("function: function <id> ( <parameter list> ) '<statement>' expected", stmt_token, statement);
    }
